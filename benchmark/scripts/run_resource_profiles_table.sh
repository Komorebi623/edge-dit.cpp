...
OUTPUT_ROOT="benchmark/results/perf-resource-profiles-$(date -u +%Y%m%d)"
...
SUITE="benchmark/configs/suites/resource-profiles.yaml"

usage() {
  cat <<'EOF'
Usage: bash benchmark/scripts/run_resource_profiles_table.sh [options]
...
Options:
  ...
  --output-root DIR       Output root for resource-profile results.
...
This script reproduces the docs/performance.md Low-VRAM deployment profile
table. The suite is FLUX.1-dev 1024x1024, 50 steps, batch 1, seed 0, load-once
generation, and six single-GPU BF16/Q8_0/Q4_K device-memory profiles.
...
make_effective_site() {
  ...
  local tmp_site="benchmark/tmp/resource-profiles-site-$(date -u +%Y%m%dT%H%M%SZ)-$$.yaml"
...
notes.append(f"Generated by run_resource_profiles_table.sh from {site_path}.")
...
def row(run):
    ...
    return (
        ...
        options.get("profile_label"),
        options.get("precision"),
        bool(options.get("keep_text_encoder_on_cpu")),
        bool(options.get("offload_to_cpu")),
        options.get("max_vram_gib"),
        bool(options.get("vae_tiling")),
    )

got = [row(run) for run in data.get("runs", [])]
expected = [
    ("flux1-dev-t2i-1024-s50", 1, None, "Performance", "bf16", False, False, None, False),
    ("flux1-dev-t2i-1024-s50", 1, None, "Memory-balanced", "q8_0", False, False, None, False),
    ("flux1-dev-t2i-1024-s50", 1, None, "24 GiB target", "q4_k", True, False, None, False),
    ("flux1-dev-t2i-1024-s50", 1, None, "16 GiB target", "q4_k", True, False, None, True),
    ("flux1-dev-t2i-1024-s50", 1, None, "12 GiB graph-budget diagnostic", "q4_k", True, True, 12, True),
    ("flux1-dev-t2i-1024-s50", 1, None, "8 GiB budget experimental", "q4_k", True, True, 8, True),
]
if got != expected:
    print("resource-profiles expansion mismatch", file=sys.stderr)
...
runs = data.get("runs", [])
print(f"resource-profiles: {len(runs)} runs")
for run in runs:
    options = run.get("run_options") or {}
    placement = []
    if options.get("keep_text_encoder_on_cpu"):
        placement.append("text-encoder-cpu")
    if options.get("offload_to_cpu"):
        placement.append("offload")
    budget = options.get("max_vram_gib")
    print(
        "  - "
        f"{options.get('profile_label')}:"
        f"{options.get('precision')}:"
        f"{'+'.join(placement) if placement else 'gpu'}:"
        f"{budget if budget is not None else 'unlimited'}GiB:"
        f"vae_tiling={'on' if options.get('vae_tiling') else 'off'}"
    )
PY
}
...
validate_resource_dry_run "$DRY_RUN_JSON"
...
cat <<EOF
Resource profile table run complete.

Result root: $OUTPUT_ROOT
Runs:        $OUTPUT_ROOT/resource-profiles
Summary:     $SUMMARY
Tables:      $TABLES
EOF
