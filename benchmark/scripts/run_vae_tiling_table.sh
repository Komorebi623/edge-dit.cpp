...
OUTPUT_ROOT="benchmark/results/perf-vae-tiling-$(date -u +%Y%m%d)"
...
SUITE="benchmark/configs/suites/vae-tiling.yaml"

usage() {
  cat <<'EOF'
Usage: bash benchmark/scripts/run_vae_tiling_table.sh [options]
...
Options:
  ...
  --output-root DIR       Output root for VAE tiling results.
...
This script reproduces the docs/performance.md VAE tiling table. The suite is
FLUX.1-dev 2048x2048, 50 steps, BF16, batch 1, seed 0, load-once generation,
and three single-GPU VAE layouts: untiled, approximately 2x2, and approximately
4x4.
...
make_effective_site() {
  ...
  local tmp_site="benchmark/tmp/vae-tiling-site-$(date -u +%Y%m%dT%H%M%SZ)-$$.yaml"
...
notes.append(f"Generated by run_vae_tiling_table.sh from {site_path}.")
...
def row(run):
    ...
    return (
        ...
        options.get("vae_mode"),
        bool(options.get("vae_tiling")),
        options.get("vae_tile_size"),
        options.get("tile_layout"),
        options.get("precision"),
    )

got = [row(run) for run in data.get("runs", [])]
expected = [
    ("flux1-dev-t2i-2048-s50", 1, None, "untiled", False, None, None, "bf16"),
    ("flux1-dev-t2i-2048-s50", 1, None, "tiled", True, 2.0, "approximately_2x2", "bf16"),
    ("flux1-dev-t2i-2048-s50", 1, None, "tiled", True, 4.0, "approximately_4x4", "bf16"),
]
if got != expected:
    print("vae-tiling expansion mismatch", file=sys.stderr)
...
runs = data.get("runs", [])
print(f"vae-tiling: {len(runs)} runs")
for run in runs:
    options = run.get("run_options") or {}
    layout = options.get("tile_layout") or "full_image"
    tile_size = options.get("vae_tile_size")
    print(
        "  - "
        f"{options.get('vae_mode')}:"
        f"{layout}:"
        f"tile_size={tile_size if tile_size is not None else '-'}:"
        f"precision={options.get('precision')}"
    )
PY
}
...
validate_vae_dry_run "$DRY_RUN_JSON"
...
cat <<EOF
VAE tiling table run complete.

Result root: $OUTPUT_ROOT
Runs:        $OUTPUT_ROOT/vae-tiling
Summary:     $SUMMARY
Tables:      $TABLES
EOF
