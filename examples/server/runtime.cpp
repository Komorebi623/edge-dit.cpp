...
bool validate_image_params(const ld_image_generation_params_t& params, std::string* error) {
...
std::string ld_status_to_string(ld_status_t status) {
    switch (status) {
        case LD_STATUS_OK: return "ok";
        case LD_STATUS_ERROR: return "error";
        case LD_STATUS_INVALID_ARGUMENT: return "invalid_argument";
        case LD_STATUS_MODEL_LOAD_FAILED: return "model_load_failed";
        case LD_STATUS_GENERATION_FAILED: return "generation_failed";
        case LD_STATUS_OUT_OF_MEMORY: return "out_of_memory";
        case LD_STATUS_UNSUPPORTED: return "unsupported";
    }
    return "unknown";
}
...
std::string ld_cache_mode_to_string(ld_cache_mode_t mode) {
    switch (mode) {
        case LD_CACHE_DISABLED: return "disabled";
        case LD_CACHE_EASYCACHE: return "easycache";
        case LD_CACHE_UCACHE: return "ucache";
        case LD_CACHE_DBCACHE: return "dbcache";
        case LD_CACHE_TAYLORSEER: return "taylorseer";
        case LD_CACHE_CACHE_DIT: return "cache-dit";
    }
    return "disabled";
}
...
bool ld_sampler_from_string(const std::string& text, ld_sampler_t* sampler) {
    ...
    static const Entry entries[] = {
        {"auto", LD_SAMPLER_AUTO},
        {"euler", LD_SAMPLER_EULER},
        {"euler-a", LD_SAMPLER_EULER_A},
        {"heun", LD_SAMPLER_HEUN},
        {"dpm2", LD_SAMPLER_DPM2},
        {"dpm-plus-plus-2s-a", LD_SAMPLER_DPM_PLUS_PLUS_2S_A},
        {"dpm++-2s-a", LD_SAMPLER_DPM_PLUS_PLUS_2S_A},
        {"dpm-plus-plus-2m", LD_SAMPLER_DPM_PLUS_PLUS_2M},
        {"dpm++-2m", LD_SAMPLER_DPM_PLUS_PLUS_2M},
        {"dpm-plus-plus-2m-v2", LD_SAMPLER_DPM_PLUS_PLUS_2M_V2},
        {"dpm++-2m-v2", LD_SAMPLER_DPM_PLUS_PLUS_2M_V2},
        {"ipndm", LD_SAMPLER_IPNDM},
        {"ipndm-v", LD_SAMPLER_IPNDM_V},
        {"lcm", LD_SAMPLER_LCM},
        {"ddim-trailing", LD_SAMPLER_DDIM_TRAILING},
        {"ddim", LD_SAMPLER_DDIM_TRAILING},
        {"tcd", LD_SAMPLER_TCD},
        {"res-multistep", LD_SAMPLER_RES_MULTISTEP},
        {"res-2s", LD_SAMPLER_RES_2S},
        {"er-sde", LD_SAMPLER_ER_SDE},
    };
...
bool ld_scheduler_from_string(const std::string& text, ld_scheduler_t* scheduler) {
    ...
    static const Entry entries[] = {
        {"auto", LD_SCHEDULER_AUTO},
        {"discrete", LD_SCHEDULER_DISCRETE},
        {"karras", LD_SCHEDULER_KARRAS},
        {"exponential", LD_SCHEDULER_EXPONENTIAL},
        {"ays", LD_SCHEDULER_AYS},
        {"gits", LD_SCHEDULER_GITS},
        {"sgm-uniform", LD_SCHEDULER_SGM_UNIFORM},
        {"simple", LD_SCHEDULER_SIMPLE},
        {"smoothstep", LD_SCHEDULER_SMOOTHSTEP},
        {"kl-optimal", LD_SCHEDULER_KL_OPTIMAL},
        {"lcm", LD_SCHEDULER_LCM},
        {"bong-tangent", LD_SCHEDULER_BONG_TANGENT},
    };
    for (const Entry& entry : entries) {
        if (value == entry.name) {
...
bool build_image_request(const json& body,
    ...
    *request = {};
    ld_image_generation_params_init(&request->params);
    ...
    if (!sampler_name.empty() && !ld_sampler_from_string(sampler_name, &request->params.sample.sampler)) {
        if (error != nullptr) {
    ...
    if (!scheduler_name.empty() && !ld_scheduler_from_string(scheduler_name, &request->params.sample.scheduler)) {
        if (error != nullptr) {
    ...
    if (!cache_mode.empty() && !ld_cache_mode_from_string(cache_mode, &request->params.sample.cache_mode)) {
        if (error != nullptr) {
...
json build_capabilities_response(const LightDitServerRuntime& runtime) {
    json result;
    result["service"] = "light-dit";
    result["model"] = runtime.display_model_path;
    result["endpoints"] = {
        "/ld/v1/health",
        "/ld/v1/models",
        "/ld/v1/capabilities",
        "/ld/v1/images/generations",
    };
    result["aliases"] = {
        "/lightdit/v1",
        "/light-dit/v1",
    ...
    result["defaults"] = {
        ...
        {"cache_mode", ld_cache_mode_to_string(runtime.defaults->cache_mode)},
    };
    return result;
}
