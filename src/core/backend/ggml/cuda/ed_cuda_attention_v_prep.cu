...
using edgedit::ggml_ext::AttentionQKVSplitPackCustomParams;
...
static bool is_contiguous_4d_f32_output(const ggml_tensor* t) {
...
__global__ void attention_qkv_split_pack_f32_kernel(const float* qkv,
                                                    float* dst,
                                                    int head_dim,
                                                    int n_head,
                                                    int seq,
                                                    ...
                                                    int64_t qkv_s2,
                                                    int64_t qkv_s3) {
    const int64_t total = static_cast<int64_t>(3) * batch * seq * n_head * head_dim;
    int64_t idx = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (idx >= total) {
        return;
    }
    ...
    const int64_t src_idx = static_cast<int64_t>(d + plane * head_dim) * qkv_s0 +
                            static_cast<int64_t>(h) * qkv_s1 +
                            static_cast<int64_t>(s) * qkv_s2 +
                            static_cast<int64_t>(b) * qkv_s3;
    const int64_t dst_idx = (((static_cast<int64_t>(b + plane * batch) * seq + s) * n_head + h) * head_dim + d);
    dst[dst_idx] = qkv[src_idx];
}
...
bool ed_cuda_attention_qkv_split_pack_custom_supported(const ggml_tensor * dst) {
    ...
    const AttentionQKVSplitPackCustomParams params = edgedit::ggml_ext::attention_qkv_split_pack_params_from_userdata(op_params.userdata);
    if (!edgedit::ggml_ext::attention_qkv_split_pack_params_valid(params)) {
        return false;
    }

    const ggml_tensor* qkv = dst->src[0];
    if (!edgedit::ggml_ext::attention_qkv_split_pack_shape_supported(qkv)) {
        return false;
    }
    if (!is_contiguous_4d_f32_output(dst)) {
        return false;
    }

    const int64_t head_dim = qkv->ne[0] / 3;
    return dst->ne[0] == head_dim &&
           dst->ne[1] == qkv->ne[1] &&
           dst->ne[2] == qkv->ne[2] &&
           dst->ne[3] == qkv->ne[3] * 3;
}
...
bool ed_cuda_attention_qkv_split_pack_custom_compute(ggml_tensor * dst, ed_cuda_attention_v_prep_stream_t stream) {
    if (!ed_cuda_attention_qkv_split_pack_custom_supported(dst)) {
        return false;
    }

    const ggml_tensor* qkv = dst->src[0];
    const int head_dim = static_cast<int>(qkv->ne[0] / 3);
    const int n_head = static_cast<int>(qkv->ne[1]);
    const int seq = static_cast<int>(qkv->ne[2]);
    const int batch = static_cast<int>(qkv->ne[3]);

    const int64_t total = static_cast<int64_t>(3) * batch * seq * n_head * head_dim;
    constexpr int threads = 256;
    const int blocks = static_cast<int>((total + threads - 1) / threads);
    attention_qkv_split_pack_f32_kernel<<<blocks, threads, 0, reinterpret_cast<cudaStream_t>(stream)>>>(
