...
namespace sd::ggml_graph_cut {
    ...
    std::string make_graph_cut_name(const std::string& group,
    ...
    ggml_tensor* leaf_tensor(ggml_cgraph* gf,
                             int leaf_index);
    ...
    bool plan_matches_graph(ggml_cgraph* gf,
                            const Plan& plan);

    ggml_tensor* output_tensor(ggml_cgraph* gf,
                               const Segment& segment,
                               size_t output_index);

    ggml_tensor* input_tensor(ggml_cgraph* gf,
                              const Segment::InputRef& input_ref);

    std::vector<ggml_tensor*> param_tensors(ggml_cgraph* gf,
                                            const Segment& segment);
    ...
    std::unordered_set<std::string> collect_future_input_names(
        ggml_cgraph* gf,
        const Plan& plan,
