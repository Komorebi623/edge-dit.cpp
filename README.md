...
- **2026-07-29:** 🚀 Added **`--auto-fit`** fully-automatic budgeting — one flag picks the DiT quantization (`q8_0 → q4_k`) *and* per-component placement to fit a hard VRAM budget (`--max-vram`), sizing headroom from a measured compute-buffer footprint at the target resolution. Supersedes `--auto-allocate` (which it implies) by also choosing quantization; at 16/24 GiB it keeps Qwen-Image's ~20 GiB DiT resident at `q4_k` instead of offloading it (~3x faster sampling). Validated across 6 families x 3 budgets (24/16/8 GiB) on an RTX 4090. See [CLI usage](docs/cli.md#budget-driven-placement---auto-allocate-and-full-auto---auto-fit).
